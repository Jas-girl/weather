    ...
    async function fn(city, state, country) {
        try {
            ...
            if (tempselect.value === "fahrenheit") {
                tempwea.innerHTML = `${Math.round(weather_data.main.temp)}°F`;
                hightemp.innerHTML = `High:${Math.round(weather_data.main.temp_max)}°F`;
                lowtemp.innerHTML = `Low:${Math.round(weather_data.main.temp_min)}°F`;
            } else {
